...
const getAllTv = async (req, res) =>{
    try{
        const tv = await Tv.find({}).sort({createAt: -1})
...
const createTv = async (req, res) =>{
    const { title, creator, firstRelease, lastRelease, seasons, season, episode, review } =req.body
    ...
    try{
        const tv = await Tv.create({ title, creator, firstRelease, lastRelease, seasons, season, episode, review })
        res.status(200).json(tv)
